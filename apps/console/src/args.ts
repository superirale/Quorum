...
/**
 * Apply `k=v` edits to an object, supporting `a.b=1` for nested keys.
 *
 * Used by `approve --set`, which is the human editing a payload before agreeing
 * to it. The result is a copy: the original proposal must stay intact, because
 * the chain records both digests and an auditor compares them.
 */
export function applyEdits(input: unknown, edits: Record<string, unknown>): unknown {
  ...
  if (input === null || typeof input !== 'object' || Array.isArray(input)) {
    throw new Error('--set needs an object payload to edit; this action proposed something else')
  }
  ...
  for (const [path, value] of Object.entries(edits)) {
    ...
    for (const part of parts.slice(0, -1)) {
      ...
      if (next === null || typeof next !== 'object' || Array.isArray(next)) {
        throw new Error(`--set ${path}: "${part}" is not an object in the proposed input`)
      }
    ...
    if (!(leaf in target)) {
      ...
      throw new Error(
        `--set ${path}: the proposed input has no "${leaf}". ` +
